...
export default async function NivelesPage() {
  ...
  return <main className="mx-auto max-w-5xl px-6 py-10 sm:py-14"><header className="mb-8 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-muted">Hola, {user.name?.split(" ")[0] ?? ""}</p><h1 className="mt-1 text-3xl font-semibold tracking-tight sm:text-4xl">Tu comunidad académica</h1></div>{niveles.length > 0 && <NuevoNivelPanel />}</header>{principal ? <><NivelPrincipal nivel={principal} />{restantes.length > 0 && <section className="mt-10" aria-labelledby="otros-niveles"><div className="mb-4 flex items-center justify-between"><h2 id="otros-niveles" className="text-lg font-medium">Otros niveles</h2><span className="text-sm text-muted">{restantes.length}</span></div><div className="grid gap-3">{restantes.map((n) => <NivelCompacto key={n.id} nivel={n} />)}</div></section>}</> : <Card className="flex flex-col items-start gap-5 rounded-[2rem] p-8 sm:p-10"><p className="font-mono text-xs font-medium tracking-[0.16em] text-ua">PRIMER PASO</p><div><h2 className="text-2xl font-semibold tracking-tight">Crea el nivel que vas a acompañar</h2><p className="mt-2 max-w-xl text-sm leading-relaxed text-muted">Solo necesitas elegir ciclo, modalidad y trimestre. Después te guiamos para sumar asignaturas, docentes y competencias.</p></div><NuevoNivelPanel abiertoPorDefecto /></Card>}</main>;
}
